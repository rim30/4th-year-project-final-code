function [label,score,cost,correct,correct2,newY,sumyes]=runkNN(noclasses,Y,Mdl,class,mu,stddev)
...
j=1;
for i=1:size(Y,1)
    c1=sqrt((mu(class,1)-Y(i,1))^2)<4*stddev(class,1);
    c2=sqrt((mu(class,2)-Y(i,2))^2)<4*stddev(class,2);
    c3=sqrt((mu(class,3)-Y(i,3))^2)<4*stddev(class,3);
    c4=sqrt((mu(class,4)-Y(i,4))^2)<4*stddev(class,4);
    c5=sqrt((mu(class,5)-Y(i,5))^2)<4*stddev(class,5);
    if c1*c2*c3*c4*c5==1
